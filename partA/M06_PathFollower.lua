...
local PathFollower = {
  ...
  init = function(self)
    ...
    self.P.next = { "" }; self.J.next = 1
    self.C.next = ""; self.D.next = ""; self.A.next = ""
    self:forward()
    self.T = initial_path -- constant for standalone, console simulation
  end,
  ...
  active = function(self)
    return self.state.curr=="NEXT"
    --[[ ******* TO COMPLETE FOR STAND-ALONE SIMULATION ******* --]]
  end
} -- PathFollower
...
if sim then
  ...
  PathFollower.init = function(self, initial_path)
    self.T = initial_path -- constant for standalone, console simulation
    self.state.prev = "NONE"
    self.state.next = "NEXT"
    self.P.next = self.T; self.J.next = 1
    self.C.next = ""; self.D.next = ""; self.A.next = ""
    self:forward()
  end -- PathFollower.init
  ...
  while PathFollower:active() do
    ...
    PathFollower:read_inputs()
    PathFollower:step()
    PathFollower:forward()
    cycle = cycle + 1
  end -- while
  PathFollower:monitor()
  PathFollower:write_outputs()
