...
init = function()
  V =  2.62 -- cm/s  -- REPLACE BY VALUES FROM CoppeliaSim
  W = 28.25 -- deg/s -- SIMULATION !!!!
...
step = function()
  ...
  if state.curr=="LISTEN" then -- TO COMPLETE !!!!
    ...
    if (T-B.curr)*V < S then
      state.next = "FWD"
    else
      state.next = "LISEN"
    end

    if state.curr == "LISTEN" and I[1] == 4 then
      M.next = "E HALT LISTEN?"
    end

    if state.curr == "FWD" and I[1] == 4 then
      M.next = string.format("D FWD HALTed, %dcm to go", math.floor(S.curr),".","The number of degrees to go is",math.floor(A.curr))
    end

    if state.curr == "TURN" and I[1] == 4 then
      M.next = string.format("D TURN HALTed, %dcm to go", math.floor(S.curr),".","The number of degrees to go is",math.floor(A.curr))
    end
